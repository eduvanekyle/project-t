...
import { ACCEPTED_EXCEL_TYPES, detectExcelFormat, mergeExcelFilesByHeader } from '../lib/excelProcessing'
...
export function SmartMergeExcel() {
    ...
    const handleMerge = async () => {
        ...
        setError(null)
        try {
            const blob = await mergeExcelFilesByHeader(items.map((item) => item.file), {
                headerRow,
                dataStartRow,
            })
    ...
    return (
        <div className="mx-auto max-w-2xl px-4 py-12 pb-28 sm:px-6">
            <ToolPageHeader
                ...
                title="Smart Merge Excel"
                description="Merge spreadsheets by matching column headers, even if they're reordered or don't all match."
            />
            ...
            {items.length > 0 && !result && (
                <div className="flex flex-col gap-4">
                    <p className="text-sm text-[var(--color-text-muted)]">
                        {items.length} file{items.length === 1 ? '' : 's'} • Columns are matched by header name,
                        regardless of order. Headers that don't match across files are kept as extra columns.
                    </p>
                    ...
                    <div className="rounded-lg border border-[var(--color-border)] bg-[var(--color-surface)] p-5">
                        ...
                        <p className="mt-1 text-sm text-[var(--color-text-muted)]">
                            Use this if your headers aren't on row 1, or if there are extra rows between the header
                            and the data.
                        </p>
